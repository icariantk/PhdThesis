//Sequence Alignment Right Enforce Left Yanking
...
namespace SARELI
{
    ...
    internal class Aligner
    {
        ...
        public Sequencer refineAlign(Sequencer Seqs, double Precision, int iteraciones)
        {
            ...
            for (int gg = 0; finiquite; gg++)
            {
                ...
                //Console.Write("\nRemoved " + move.count() + " sequences of " + dd + "\n");
                Aligner refi = new Aligner();
                for (int c = 0; c != move.count(); c++)
                {
                    string s = move.getHeader(c) + "\n" + move.getSequence(c);
                    Seqs = refi.cudaAlignPSP(Seqs, new Sequencer(s, false));

                    // Console.Write("\r                      \rRealigned: " + c);
                }
                /*
                now = new Aligner().columnScore(Seqs);
                if (now < initial)
                {
                    Console.Write("\nColumn score dimished, aborting\n");
                    return backup;
                }
                 */
            }
            /*if (new Aligner().columnScore(Seqs) < initial)
            {
             *
                finiquite = false;
                Seqs = backup;
        ...
        public void align2(String submatrix = "blosum62", int seqX = 0, int seqY = 1)
        {
            ...
            if (seqX != seqY && seqToAlign.count() > 1 && seqX < seqToAlign.count() && seqY < seqToAlign.count())
            {
                ...
                for (int c = 0; c != seqToAlign.sequences[seqX].Count; c++)
                {
                    matrix[c][0] = 0;
                    matrixDir[c][0] = -1;
                }
                ...
                {
                    matrix[c][0] = gap("gap opening") * c;
                }
                for (int c = 0; c != matrix[0].Count(); c++)
                {
                    matrix[0][c] = gap("gap opening") * c;
                }
                for (int c = 0; c != matrix.Count(); c++)
                {
                    for (int d = 0; d != matrix[c].Count(); d++)
                    {
                        if (d == 0)
                        {
                            matrixDir[c][d] = 2;
                ...
                {
                    int a;      //match/mismatch in the diagonal
                    int b;      //gap in sequence x
                    int c;      //gap in sequence y
                    ...
                    for (int i = 1; i != seqToAlign.sequences[seqX].Count + 1; i++)
                    {
                        ...
                        {
                            if (seqToAlign.sequences[seqX].ElementAt(i - 1) != '-' && seqToAlign.sequences[seqY].ElementAt(j - 1) != '-')
                            {
                                a = matrix[i - 1][j - 1] + Blosum62[seqToAlign.sequences[seqX].ElementAt(i - 1) - 65][seqToAlign.sequences[seqY].ElementAt(j - 1) - 65];
                            }
                            else
                            {
                                a = gap(2);
                            }
                            b = matrix[i - 1][j] + gap(2);
                            c = matrix[i][j - 1] + gap(2);

                            if (a >= c && a >= b) { dir = 1; score = a; }
                            if (b > a && b >= c) { dir = 2; score = b; }
                            if (c > a && c > b) { dir = 3; score = c; }
                    ...
                    while (!(x == 0 && y == 0))
                    {
                        if (matrixDir[x][y] == 1) { --x; --y; }
                        else
                            if (matrixDir[x][y] == 2) { seqToAlign.sequences[seqY].Insert(seqToAlign.sequences[seqY].Count - y, '-'); --x; }
                            else
                                if (matrixDir[x][y] == 3) { seqToAlign.sequences[seqX].Insert(seqToAlign.sequences[seqX].Count - x, '-'); --y; }
                    }
                }
            }
            else
            {
                Console.Write("Error in sequence selection");
            }
        }
        ...
        /// <returns></returns>
        public Sequencer alignByPSP(Sequencer A, Sequencer B, String submatrix = "blosum62")
        {
            ...
            for (int x = 1; x != ka; x++)
            {
                for (int y = 1; y != kb; y++)
                {
                    int tl = matrix[x - 1][y - 1] + sumOfPairs(A, B, x - 1, y - 1);
                    int t = matrix[x][y - 1] + sumOfPairs(A, B, -1, y - 1);
                    int l = matrix[x - 1][y] + sumOfPairs(A, B, x - 1, -1);
            ...
            while (!(i == 0 && j == 0))
            {
                if (matrixDir[j][i] == 3)
                {
                    ...
                    for (int c = 0; c != na; c++) { A.sequences[c].Insert(j, '-'); }
                ...
                if (matrixDir[j][i] == 2)
                {
                    ...
                    for (int c = 0; c != nb; c++) { B.sequences[c].Insert(i, '-'); }
            ...
            */
            ...
            while (N > 1)
            {
                ...
                if (headers[indexSmallestX] >= NN)
                {
                    ...
                    {
                        aligning[T - NN] = worker2.cudaAlignPSP(aligning[headers[indexSmallestX] - NN], aligning[headers[indexSmallestY] - NN], "simple");
                    }
                    else
                    {
                        aligning[T - NN] = worker2.cudaAlignPSP(aligning[headers[indexSmallestX] - NN], allDivided[headers[indexSmallestY]], "simple");
                    }
                }
                else
                {
                    if (headers[indexSmallestY] >= NN)
                    {
                        aligning[T - NN] = worker2.cudaAlignPSP(allDivided[headers[indexSmallestX]], aligning[headers[indexSmallestY] - NN], "simple");
                    }
                    else
                    {
                        aligning[T - NN] = worker2.cudaAlignPSP(allDivided[headers[indexSmallestX]], allDivided[headers[indexSmallestY]], "simple");
                    }
                }
                T++;
                double sumsmx = 0;
            ...
            */
            ...
            while (N > 1)
            {
                ...
                if (headers[indexSmallestX] >= NN)
                {
                    if (headers[indexSmallestY] >= NN)
                    {
                        aligning[T - NN] = worker2.alignByPSP(aligning[headers[indexSmallestX] - NN], aligning[headers[indexSmallestY] - NN]);
                    }
                    else
                    {
                        aligning[T - NN] = worker2.alignByPSP(aligning[headers[indexSmallestX] - NN], allDivided[headers[indexSmallestY]]);
                    }
                }
                else
                {
                    if (headers[indexSmallestY] >= NN)
                    {
                        aligning[T - NN] = worker2.alignByPSP(allDivided[headers[indexSmallestX]], aligning[headers[indexSmallestY] - NN]);
                    }
                    else
                    {
                        aligning[T - NN] = worker2.alignByPSP(allDivided[headers[indexSmallestX]], allDivided[headers[indexSmallestY]]);
                    }
                }
                T++;
                double sumsmx = 0;
        ...
        public double distancePropossal(int x = 0, int y = 1, int radious = 3)
        {
            ...
            int cy = seqToAlign.sequences[y].Count;
            int cx = seqToAlign.sequences[x].Count;
            int pen = 0;
            ...
            if (k != -1)
            {
                for (int c = 0; c != cx; c++)
                {
                    for (int d = c - radious; d != c + radious; d++)
                    {
                        if (d >= 0 && d < cy)
                        {
                            if (seqToAlign.sequences.ElementAt(x).ElementAt(c) != '-' && seqToAlign.sequences.ElementAt(y).ElementAt(d) != '-')
                            {
                                pen = 0;
                                if (d < c) { sum += 1.0 / (Math.Abs(((1.0 + (d - (c - radious))) / (radious + 1))) * (Blosum62[seqToAlign.sequences.ElementAt(x).ElementAt(c) - 65][seqToAlign.sequences.ElementAt(y).ElementAt(d) - 65] - gap(1) + 4)); }
                                if (d == c) { sum += 1.0 / ((Blosum62[seqToAlign.sequences.ElementAt(x).ElementAt(c) - 65][seqToAlign.sequences.ElementAt(y).ElementAt(d) - 65] - gap(1) + 4)); }
                                if (d > c) { sum += 1.0 / (1.0 - ((d - c) / (radious + 1)) * (Blosum62[seqToAlign.sequences.ElementAt(x).ElementAt(c) - 65][seqToAlign.sequences.ElementAt(y).ElementAt(d) - 65] - gap(1) + 4)); }
                            }
                            else
                            {
                                if (pen == 0)
                ...
                if (x < ka && y < kb)
                {
                    for (int i = 0; i != na; i++)
                    {
                        for (int j = 0; j != nb; j++)
                        {
                            ...
                            {
                                sum += Blosum62[A.sequences[i][x] - 65][B.sequences[j][y] - 65];
                            }
                            else
                            {
                                if (x > 1 && y > 1)
    ...
    internal class ZProgram
    {
        ...
        private static int compare(string[] args)
        {
            ...
            for (int c = 0; c != args.Count(); c++)
            {
                if (args[c].Substring(0, 4) == "aln:")
                {
                    Sequencer prefab = new Sequencer(args[c].Substring(4, args[c].Length - 4), true);
                    Aligner test1 = new Aligner(prefab);

                    if (prefab.count() != 0)
                    {
                        //test1.centerStarAllvsAll();
                        //  Console.WriteLine("Sum of pairs: " + test1.SumOfPairs());
                        // Console.WriteLine("Sum of pairs: " + test1.BLOSUM62());
                        //timer.Start();
                        //test1.align2Cuda();
                        //timer.Stop();
                        //Console.WriteLine("Cuda:"+timer.ElapsedTicks+"\nCPU");
                        //timer.Reset();
                        //timer.Start();
                        //test1.align2();
                        //timer.Stop();
                        //Console.WriteLine("CPU:" + timer.ElapsedTicks);
                        /*
                                            for (int cc = 0; cc != test1.seqToAlign.count(); cc++) {
                                                Console.WriteLine(test1.seqToAlign.getSequence(cc) + "\n");
                                            }
                                            */
                    }
                ...
                else
                {
                   ...
                   // if (!File.Exists("muscle_" + file + ".fasta"))
                    if (!File.Exists("muscle_" + file + ".fasta"))
                    {
                        ...
                        string path = Path.GetDirectoryName(args[c]);
                        Aligner worker = new Aligner(all);
                        Sequencer NHT;
                        Sequencer[] NHT_2 = new Sequencer[9];
                        Stopwatch a = new Stopwatch();
                        Aligner scorer;
                        ...
                        if (File.Exists(path2 + ".\\Results.txt"))
                        {
                            System.IO.StreamReader X = new System.IO.StreamReader(path2 + ".\\Results.txt");
                            string line;
                            int count = 0;
                        ...
                        if (radios.ContainsKey(file))
                        {
                            Console.WriteLine("Checando: " + file);
                            if (!skipping.Contains(file))
                            {
                                int k = radios[file];
                                worker.gap(-2, -4, -3);
                                bool po = true;
                                ...
                                while (po && n > 0)
                                {
                                    try
                                    {
                                        NHT_2[k - 3] = worker.alignByNeighbourJoiningCUDA(all, "propossal", k, n);
                                        po = false;
                                    }
                                    catch (Exception e)
                                    {
                                        Console.WriteLine("No cupo D:  tamaño=" + n);
                                    }
                                    n -= 6;
                                }
                                if (n < 0)
                                {
                                    Console.WriteLine("Esta secuencia no es apta para esta tarjeta de video");
                                    return 0;
                                }
                                worker = new Aligner(NHT_2[k - 3]);
                                nowScore = worker.sumOfPairs();
                                header += "SP Propossal,";
                                tofile += String.Format("{0:0}", nowScore) + ",";
                                nowScore = worker.columnScore(NHT_2[k - 3]);
                                header += "CS Propossal,";
                                tofile += String.Format("{0:0}", nowScore) + ",";
                                Double refiA, refiB;
                                for (refiA = 1; refiA > .1; refiA -= .1)
                                {
                                    for (refiB = refiA; refiB >= .1; refiB -= .1)
                                    {
                                        Console.WriteLine(refiA + "/" + refiB);
                                        Sequencer temp = worker.refineAlign(new Sequencer(NHT_2[k - 3].getFasta(), false), refiA, 5);
                                        Sequencer temp2 = worker.refineAlign2(new Sequencer(temp.getFasta(), false), refiB);
                                        header += "SP Refined (" + refiA + "/" + refiB + "),";
                                        nowScore = new Aligner(temp2).sumOfPairs();
                                        tofile += String.Format("{0:0}", nowScore) + ",";
                                        header += "CS Refined (" + refiA + "/" + refiB + "),";
                                        nowScore = worker.columnScore(temp2);
                                        tofile += String.Format("{0:0}", nowScore) + ",";
                                    }
                                }
                                maxRSP = k;
                                ...
                                /*
                                    header += "SP Refined,";
                                    nowScore = new Aligner(temp).sumOfPairs();
                                    tofile += String.Format("{0:0}", nowScore) + ",";
                            ...
                            {
                                Console.WriteLine(file + " Skipped :3");
                            }
                        }
                        else
                        {
                            for (int k = 3; k != 11; k++)
                            {
                                ...
                                //NHT_2[k - 3] = worker.alignByNeighbourJoiningCUDA(all, "propossal", k, 12);
                                a.Stop();
                                header += "Cuda NJ proposed time (ms) radio:" + k + ",";
                                tofile += a.ElapsedMilliseconds + ",";

                                worker = new Aligner(NHT_2[k - 3]);
                                nowScore = worker.sumOfPairs();
                                header += "SP NJ Propossal CUDA radio:" + k + ",";
                                tofile += String.Format("{0:0}", nowScore) + ",";

                                nowScore = worker.columnScore(NHT_2[k - 3]);
                                header += "CS NJ Propossal CUDA radio:" + k + ",";

                                tofile += String.Format("{0:0}", nowScore) + ",";
                                Sequencer temp = worker.refineAlign(new Sequencer(NHT_2[k - 3].getFasta(), false), refi, 5);

                                NHT_2[k - 3] = worker.refineAlign2(temp, refi / 2.0);
                                //NHT_2[k - 3] = worker.refineAlign3(NHT_2[k - 3],temp, refi);
                                //NHT_2[k - 3].print(0, -1, false, ".\\SARELY_Ref_CS_" + k + "_" + file + ".fasta");
                                header += "SP Refined NJ Propossal CUDA radio:" + k + ",";
                                nowScore = new Aligner(NHT_2[k - 3]).sumOfPairs();
                                int t = (int)nowScore;
                                tofile += String.Format("{0:0}", nowScore) + ",";
                                if (nowScore > maxRefSP || k == 3)
                                {
                                    maxRefSP = (int)nowScore;
                                    maxRSP = k;
                                    maxRSP_cs = (int)worker.columnScore(NHT_2[k - 3]);
                                }
                                nowScore = worker.columnScore(NHT_2[k - 3]);
                                if (nowScore > maxRefCS || k == 3)
                                {
                                    maxRefCS = (int)nowScore;
                                    maxRCS_sp = t;
                                    maxRCS = k;
                                ...
                                header += "CS Refined NJ Propossal CUDA radio:" + k + ",";
                                tofile += String.Format("{0:0.00}", nowScore) + ",";
                            }
                        }

                        /*
                        NHT_2[maxRCS - 3].print(0, -1, false, ".\\SARELY_Ref_CS_" + maxRCS + "_" + file + ".fasta");
                        NHT_2[maxRSP - 3].print(0, -1, false, ".\\SARELY_Ref_SP_" + maxRSP + "_" + file + ".fasta");

                        header += "Max CS score,Max CS_SP score,Max SP_CS score,Max SP score, R CS, R SP,";
                        tofile += maxRefCS + "," + maxRCS_sp + "," + maxRSP_cs + "," + maxRefSP + "," + maxRCS + "," + maxRSP + ",";

                        //nowScore = new Aligner(new Sequencer(".\\SARELY_Ref2_CS_7_BB40004.fasta", true)).sumOfPairs();
                        //Console.WriteLine(file + " CS=" + maxRefCS + " R=" + maxRCS + "Sum of pairs before: "+maxRSP+" after refinement: cs="+worker.columnScore(test)+" sp= "+nowScore);

                        worker = new Aligner(all);

                        if (File.Exists(file + ".msf"))
                        {
                            NHT = new Sequencer(file + ".msf", true);
                            scorer = new Aligner(NHT);
                            nowScore = 0;
                            nowScore = scorer.sumOfPairs();
                            header += "SP Database,";
                            tofile += nowScore + ",";
                            header += "CS Database,";
                            nowScore = worker.columnScore(NHT);
                            tofile += String.Format("{0:0}", nowScore) + ",";
                        }
                        else
                        {
                        ...
                        command.StartInfo.FileName = path2+"\\clustalw2.exe";
                        command.StartInfo.Arguments = "\"" + args[c] + "\"";
                        ...
                        */
                        ...
                        if (!File.Exists(path2 + "\\Results.txt"))
                        {
                            StreamWriter res = new StreamWriter(path2 + "\\Results.txt", true);
            ...
            {
                Console.WriteLine("                         ┌────────────────────────────────────────────────────┐");
                Console.WriteLine("                         │      Score-Align-Record Enhanced Lineup System     │");
                Console.WriteLine("                         │                     SARELI System                  │");
                Console.WriteLine("                         ├────────────────────────────────────────────────────┤");
                ...
                Console.WriteLine("     │     least one, in any of the supported file formats enumerated in the next section. The tasks  │");
                Console.WriteLine("     │     selected by the operators will operate on each of the input files, the default output      │");
                Console.WriteLine("     │     is a file in FASTA format, with the prefix SARELI_ on the name of the file, by default     │");
                Console.WriteLine("     │     output format is fasta (e.g. SARELI_ORIGINALFILENAME.FASTA) and a statistic file           │");
                Console.WriteLine("     │     Results.txt with the information about the tasks performed                                 │");
                Console.WriteLine("     └────────────────────────────────────────────────────────────────────────────────────────────────┘\n\n");
                Console.WriteLine("     ┌────────────────────────────────────────────────────────────────────────────────────────────────┐");
                ...
                Console.WriteLine("     │                                                                                                │");
                Console.WriteLine("     │              SARELI [-s SS] [-a AA] [-sm file] [-o OF] file...                                 │");
                Console.WriteLine("     │                                                                                                │");
                Console.WriteLine("     │       Options:                                                                                 │");
                Console.WriteLine("     │                                                                                                │");
                Console.WriteLine("     │              -s SS                                                                             │");
                Console.WriteLine("     │                   Score scheme to be used to report results                                    │");
                Console.WriteLine("     │                                                                                                │");
                Console.WriteLine("     │              -a AA                                                                             │");
                Console.WriteLine("     │                   Align Algorithm to be used to align sequences                                │");
                Console.WriteLine("     │                                                                                                │");
                Console.WriteLine("     └────────────────────────────────────────────────────────────────────────────────────────────────┘\n\n");
            }
...
//---------------------------           align2 CUDA Kernel             ---------------------------
//  Fill the matrix and matrixDir using using Needleman/Wunsch dinamic programming in a[m] and b[n] int arrays representing the sequences and the
//  scorematrix array used to score the aminoacids, a[m] and b[n] integers arrays with english ascii characters i.e. A=65, B=66, in each element.
//--------------------------------------------------------------------------------------------------
...
//----------------------------------Simple gap function for penalties-------------------------------
//                                       used in align2 kernel
//--------------------------------------------------------------------------------------------------
...
//------------------------------------Invert CUDA Kernel--------------------------------------------
//         Used for invert the traceback sequence, saved the result in a and b int arrays
//--------------------------------------------------------------------------------------------------
...
//------------------------------------Traceback CUDA kernel-----------------------------------------
//         Follow the rute from the M,N matrix, to 0,0 position in the matrixDir direction.
//         saves the aligned and inverted sequence in ai and bi int arrays
//--------------------------------------------------------------------------------------------------
...
__global__ void traceback(int a[], int ai[], int b[],int bi[], int matrixDir[],int m,int n, int *k)
{
	...
	while(!(x==0&&y==0)){
		if(matrixDir[y*m+x]==3){ai[c]='-';if(y>0){bi[c]=b[y-1];y--;}}else
			if(matrixDir[y*m+x]==2){bi[c]='-';if(x>0){ai[c]=a[x-1];x--;}}else
				if(matrixDir[y*m+x]==1){if(x>0&&y>0){ai[c]=a[x-1];bi[c]=b[y-1];x--;y--;}}else{x=0;y=0;}
		c++;
	}
	ai[c]='\0';
	bi[c]='\0';
